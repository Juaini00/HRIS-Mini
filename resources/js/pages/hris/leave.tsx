...
type Props = {
  requests: { data: Array<any> };
  types: Array<any>;
  balances: Array<any>;
  canReview: boolean;
};
export default function Leave({ requests, types, balances, canReview }: Props) {
  return (
    <>
      <Head title="Cuti" />
            ...
            <Card key={b.id}>
              <CardContent className="p-5">
                ...
                <p className="text-2xl">
                  {Number(b.entitled) - Number(b.used) - Number(b.pending)} hari
                </p>
            ...
            >
              {({ processing, errors }) => (
                <>
                  ...
                  <Button disabled={processing}>Kirim pengajuan</Button>
              ...
              <tbody>
                {requests.data.map((r) => (
                  <tr key={r.id} className="border-b">
                    <td className="p-4">{r.employee?.user?.name ?? 'Saya'}</td>
                      ...
                      {canReview && r.status === 'pending' && (
                        <div className="flex gap-2">
                          <Button
                            size="sm"
                            onClick={() =>
                              router.patch(`/leave/${r.id}/review`, {
                                status: 'approved',
                              })
                            }
                          >
