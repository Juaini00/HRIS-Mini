...
test('daily absence processing is idempotent and respects leave holiday and weekend', function () {
    ...
    LeaveRequest::create(['employee_id' => $employee->id, 'leave_type_id' => $type->id, 'start_date' => '2026-08-10', 'end_date' => '2026-08-10', 'days' => 1, 'reason' => 'Rest', 'status' => LeaveRequestStatus::Approved]);
    $this->artisan('nusahr:process-absences', ['date' => '2026-08-10'])->assertSuccessful();
    $this->artisan('nusahr:process-absences', ['date' => '2026-08-10'])->assertSuccessful();
    expect(Attendance::where('employee_id', $employee->id)->where('date', '2026-08-10')->sole()->status)->toBe(AttendanceStatus::OnLeave)->and(Attendance::count())->toBe(1);
...
test('missing checkout becomes incomplete', function () {
    Date::setTestNow('2026-08-07 23:30:00');
    $employee = attendanceEmployee();
    ...
    expect(Attendance::sole()->status)->toBe(AttendanceStatus::Incomplete);
});
