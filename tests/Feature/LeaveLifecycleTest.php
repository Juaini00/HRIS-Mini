...
test('half day leave reserves half a day and cancellation restores it', function () {
    ...
    expect((float) $balance->refresh()->pending)->toBe(0.0)->and($leave->refresh()->status)->toBe(LeaveStatus::Cancelled);
});

test('cancelling an approved future leave restores used balance', function () {
    Notification::fake(); Date::setTestNow('2026-08-07 08:00:00');
    $employee = leaveLifecycleEmployee(); $hr = User::factory()->create(['role' => UserRole::HrAdmin]); [$type, $balance] = leaveLifecycleBalance($employee);
    ...
    app(ReviewLeaveRequest::class)->handle($leave, $hr, LeaveStatus::Approved);
...
test('review dispatches notification and manager cannot approve own request', function () {
    Notification::fake(); Date::setTestNow('2026-08-07 08:00:00');
    $manager = leaveLifecycleEmployee(UserRole::Manager); [$type] = leaveLifecycleBalance($manager);
    ...
    expect(fn () => app(ReviewLeaveRequest::class)->handle($leave, $hr, LeaveStatus::Approved))
