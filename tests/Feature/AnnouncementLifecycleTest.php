...
test('draft scheduled and unrelated audience announcements are hidden', function () {
    ...
    $author = User::factory()->create(['role' => UserRole::HrAdmin]);
...
test('scheduled publishing notifies matching audience once', function () {
    Notification::fake();
    $employee = announcementEmployee();
    $manager = announcementEmployee(UserRole::Manager);
    $author = User::factory()->create(['role' => UserRole::HrAdmin]);
    ...
    $this->artisan('nusahr:publish-announcements')->assertSuccessful();
    $this->artisan('nusahr:publish-announcements')->assertSuccessful();
    Notification::assertSentToTimes($employee->user, AnnouncementPublishedNotification::class, 1);
    Notification::assertNotSentTo($manager->user, AnnouncementPublishedNotification::class);
