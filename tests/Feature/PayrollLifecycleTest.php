...
function payrollEmployee(float $salary = 10_000_000): Employee
{
    ...
    $position = Position::create(['department_id' => $department->id, 'name' => fake()->jobTitle()]); $user = User::factory()->create();
...
test('payroll calculates fixed percentage and absence deductions deterministically', function () {
    ...
    $action = app(GeneratePayrollPeriod::class); $period = $action->handle(['name' => 'August 2026', 'starts_on' => '2026-08-01', 'ends_on' => '2026-08-31']);
    $first = $period->records->first()->net_salary; $second = $action->handle(['name' => 'August 2026', 'starts_on' => '2026-08-01', 'ends_on' => '2026-08-31'])->records->first()->net_salary;
...
test('employee sees only own published payroll and receives publication notification', function () {
    Notification::fake(); $employee = payrollEmployee(); $other = payrollEmployee(); $admin = User::factory()->create(['role' => UserRole::HrAdmin]);
    ...
    Notification::assertSentTo($employee->user, PayrollPublishedNotification::class); Notification::assertSentTo($other->user, PayrollPublishedNotification::class);
...
test('published payroll cannot be regenerated or adjusted', function () {
    $employee = payrollEmployee(); $admin = User::factory()->create(['role' => UserRole::HrAdmin]);
    $period = app(GeneratePayrollPeriod::class)->handle(['name' => 'August', 'starts_on' => '2026-08-01', 'ends_on' => '2026-08-31']);
    $period->update(['status' => PayrollStatus::Published]);
    expect(fn () => app(GeneratePayrollPeriod::class)->handle(['name' => 'August', 'starts_on' => '2026-08-01', 'ends_on' => '2026-08-31']))->toThrow(Illuminate\Validation\ValidationException::class);
...
test('employee cannot view draft or another employees payslip', function () {
    $employee = payrollEmployee(); $other = payrollEmployee();
    $period = app(GeneratePayrollPeriod::class)->handle(['name' => 'August', 'starts_on' => '2026-08-01', 'ends_on' => '2026-08-31']);
    $own = $period->records->firstWhere('employee_id', $employee->id); $otherRecord = $period->records->firstWhere('employee_id', $other->id);
