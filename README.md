...
**Every seeded account uses the password `password`.** The seeder is deterministic,
so these exact emails exist after `php artisan migrate:fresh --seed` — no need to read
the seeder to find a login.

Primary account per role (start here):

| Email | Password | Role | Name |
| --- | --- | --- | --- |
| `admin@nusahr.test` | `password` | Super Admin | Ratna Wijaya |
| `hr@nusahr.test` | `password` | HR Admin | Dewi Kartika |
| `manager@nusahr.test` | `password` | Manager | (seeded) |
| `employee@nusahr.test` | `password` | Employee | (seeded) |

All 49 accounts (all password `password`):

| Role | Count | Emails |
| --- | :-: | --- |
| Super Admin | 1 | `admin@nusahr.test` |
| HR Admin | 2 | `hr@nusahr.test`, `hr.admin@nusahr.test` (Bayu Pratama) |
| Manager | 6 | `manager@nusahr.test`, then `manager1@nusahr.test` … `manager5@nusahr.test` |
| Employee | 40 | `employee@nusahr.test`, then `employee1@nusahr.test` … `employee39@nusahr.test` |

Manager and employee display names are generated by a fixed-seed faker, so they are
stable across runs but not listed here — sign in and the name shows on the profile.
`employee@nusahr.test` and a few of the `employeeN` accounts are seeded in non-active
states (probation, on-leave, suspended, resigned, terminated) so dashboards and reports
have a realistic spread; a non-active account still logs in but the app gates it per its
status.
